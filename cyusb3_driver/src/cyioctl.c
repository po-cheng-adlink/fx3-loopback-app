    ...
    )
{
	...
	if (NT_SUCCESS(NtStatus))
	{
        ...
        //DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "CyUSB3 : Number of bytes read: %d\n", pMainReqContext->ulBytesReadWrite);
    }
	else
	{
		CyTraceEvents(TRACE_LEVEL_ERROR, DBG_IOCTL,
        ...
        //DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "CyUSB3 : ReadWrite failed - request status 0x%x UsbdStatus 0x%x\n",
        //        NtStatus, TrafrCompletionParams->UsbdStatus);
