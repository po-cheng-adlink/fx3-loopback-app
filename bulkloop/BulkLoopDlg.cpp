...
void CBulkLoopAppDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Control(pDX, IDC_EDT_SUCCESS, m_edtSuccess);
    DDX_Control(pDX, IDC_EDT_SUCCESS2, m_edtFailure);
}
...
BOOL CBulkLoopAppDlg::OnInitDialog()
{
    ...
    m_edtSuccess.SetWindowText(_T("0"));
    m_edtFailure.SetWindowText(_T("0"));
...
DWORD WINAPI CBulkLoopAppDlg::PerformBulkloopTransfer(LPVOID lParam)
{
    ...
    pThis->m_edtSuccess.SetWindowText(L"0x0");
    pThis->m_edtFailure.SetWindowText(L"0x0");
    ...
    while (m_bBulkLoopCompleted == FALSE )
    {
        ...
        strBytes.Format(L"0x%X", m_nSuccess);
        pThis->m_edtSuccess.SetWindowText(strBytes);
        strBytes.Format(L"0x%X", m_nFailure);
        pThis->m_edtFailure.SetWindowText(strBytes);
        ...
        if (epBulkIn->NtStatus || epBulkIn->UsbdStatus) 
        {
            ...
            m_nFailure++;
            CString strMsg;
            strMsg.Format(L"0x%X", m_nFailure);
            pThis->m_edtFailure.SetWindowText(strMsg);
...
LRESULT CBulkLoopAppDlg::OnUserAppClose(WPARAM wParam, LPARAM lParam)
{
    ...
    else if (m_bDeviceChanging == TRUE )
    {
        ...
        m_edtSuccess.SetWindowText(L"0x0");
        m_edtFailure.SetWindowText(L"0x0");
